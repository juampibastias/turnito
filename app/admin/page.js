...
export default function BookingPage() {
    ...
    const fetchAvailableSlots = async (date, duration) => {
        try {
            ...
            if (Array.isArray(data)) {
                console.log(
                    '[FRONT] Slots recibidos (array válido):',
                    data.length,
                    'slots'
                );
                setAvailableSlots(data);
            } else {
                // Si no es un array, mostrar error y establecer array vacío
                console.error('[FRONT] Error: respuesta no es array:', data);
                setAvailableSlots([]);

                // Mostrar mensaje al usuario si hay error específico
                if (data.message) {
                    console.log(`[FRONT] API Error: ${data.message}`);
                    // Opcional: alert(`Error: ${data.message}`);
                }
            }
        } catch (error) {
            console.error('[FRONT] Error fetching available slots:', error);
            setAvailableSlots([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleDateSelect = (date) => {
        console.log('[FRONT] Fecha seleccionada:', date);
    ...
    const handleTimeSlotSelect = (slot) => {
        console.log('[FRONT] Slot seleccionado:', slot);
    ...
    const getDepositAmount = () => {
        return getTotalPrice() * 0.5; // 50% de seña
    };
                        ...
                        {step === 3 && (
                            <div>
                                ...
                                {loading ? (
                                    ...
                                    <div className='mb-6'>
                                        {/* ✅ VALIDACIÓN CRÍTICA: Triple verificación antes del .map() */}
                                        {availableSlots &&
